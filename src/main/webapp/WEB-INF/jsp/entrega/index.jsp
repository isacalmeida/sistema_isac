...
<div class="wrapper">
	...
	<div class="content-wrapper">
		<section class="content-header">
			<h1>
				Entrega
				<small>Cadastro de entregas dos pedidos</small>
			</h1>
			<ol class="breadcrumb">
							...
							<c:when test="${var == 0}"> 
								<c:choose>
									...
									<c:when test="${acao == 2 }">
										<div class="callout callout-danger">
											<h4><i class="icon fa fa-ban"></i> Ops!</h4>
											<p>Entrega com relacionamentos ativos.</p>
										</div>
									</c:when>
								</c:choose>
							</c:when>
					...
					<div class="box-header with-border">
						<div class="col-xs-10 col-sm-6 col-md-4 col-lg-4">
							<c:if test="${permissao != 2 }">
								<a href="<c:url value='/entrega/novo'/>" ><button type="button" class="btn btn-primary btn-flat">Novo</button></a>
							</c:if>
							...
							<tbody>
								<c:forEach var="e" items="${entregas }">
									<c:if test="${!empty(e)}" >
										<tr>
											<td>${e.codigo }</td>
											<td><a href="<c:url value='/entrega/${e.codigo }/editar' />" >${e.status }</a></td>
											<td>${e.cod_pedido }</td>
											<td>${e.criacao }</td>
											<td>${e.alteracao }</td>
										</tr>
