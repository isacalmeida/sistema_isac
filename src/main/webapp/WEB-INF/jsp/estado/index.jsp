							...
							<c:when test="${var == 0}"> 
								<c:choose>
									...
									<c:when test="${acao == 2 }">
										<div class="callout callout-danger">
											<h4><i class="icon fa fa-ban"></i> Ops!</h4>
											<p>Estado com relacionamentos ativos.</p>
										</div>
									</c:when>
								</c:choose>
							</c:when>
					...
					<div class="box-header with-border">
						<div class="col-xs-10 col-sm-6 col-md-4 col-lg-4">
							<c:if test="${permissao != 2 }">
								<a href="<c:url value='/estado/novo'/>" ><button type="button" class="btn btn-primary btn-flat">Novo</button></a>
							</c:if>
							...
							<tbody>
								<c:forEach var="e" items="${estados }">
									<c:if test="${!empty(e)}" >
										<tr>
											<td>${e.codigo }</td>
											<td><a href="<c:url value='/estado/${e.codigo }/editar' />" >${e.descricao }</a></td>
											<td>${e.sigla }</td>
											<td>${e.criacao }</td>
										</tr>
									</c:if>
